...
class StringValidator(BaseValidator):
    ...
    def verify(self, value):
        ...
        if not isinstance(value, str):
            self.metadata.warning_func(
                f"Property {self.path} was expected to be of type string"
            )
            value = str(value)
        if self.patterns:
            if not any(pattern.match(value) for pattern in self.patterns.values()):
                self.metadata.error_func(
                    f"Property {self.path} is {value} but must match one of {list(self.patterns.keys())}"
                )
                value = self.layout.get("default", "")
        return value
    ...
    def check(self, value):
        if not self.patterns:
            return
        match = next(
            (key for key, pattern in self.patterns.items() if pattern.match(value)),
            None,
        )
        ...
        for forbidden in checks.get("forbid", []):
            ...
            if self.metadata[other_path] == expected:
                self.metadata.error_func(
                    f"Property {self.path} has value {value} which forbids property {other_path} to have value {expected}"
                )
        for required in checks.get("required", []):
            if not self.metadata[
                required
            ]:  # TODO: This is not a good way to handle this check I think
                self.metadata.error_func(
                    f"Property {self.path} has value {value} which requires property {required}"
                )
        if "warn" in checks:
...
class ObjectValidator(BaseValidator):
    def verify(self, value):
        ...
        if self.layout.get("parsing") == "legacy-validation":
            if not isinstance(value, str):
                self.metadata.error_func(
                    f"Property {self.path} was expected to be a string"
                )
                return {}
        ...
        if not isinstance(value, dict):
            self.metadata.error_func(
                f"property {self.path} was expected to be a dictionary"
            )
            return {}
        for prop in self.layout.get("required", []):
            if prop not in value:
                self.metadata.error_func(
                    f"Missing required property: {self.path}/{prop}"
                )
    ...
    def verify(self, value):
        ...
        if not isinstance(value, float):
            try:
                ...
                self.metadata.error_func(
                    f"Property {self.path} was expected to be a float"
                )
    ...
    def verify(self, value):
        ...
        if not isinstance(value, int):
            try:
                value = int(value)
                self.metadata.warning_func(
                    f"Property {self.path} should be of type integer, interpreting as {value}"
                )
            except Exception:
                self.metadata.error_func(
                    f"Property {self.path} was expected to be an integer"
                )
