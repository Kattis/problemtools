...
1.  Create a new directory within `tests/default_validator_tests/` (e.g., `tests/default_validator_tests/test_my_new_feature_ac`).
...
4.  Use the `run_and_generate_expected.py` script to automatically generate `expected_exit_code.txt` and optionally `expected_message.txt`:
    ```bash
    cd tests/default_validator_tests/
    ./run_and_generate_expected.py test_my_new_feature_ac
    ```
